...
export default class ForgottenPasswordRequest extends React.Component {
	state = {
		email: '',
		success: false,
		error: false,
	...
	submitForm = async () => {
		try {
			const result = await axios.post(`${serverConfig.url}users/forgotten-password`, this.state);
				...
				<div className="card border-0 shadow my-5 p-5">
					<Conditional if={this.state.success}>
						<div className="alert alert-success">
							Password reset email sent if an account exists for the email address provided.
						</div>
					</Conditional>
