...
export class Event extends Component {
  ...
  firstThreeRecommendations = () =>
    this.state.recommendations.filter((v, i) => i < 3);

  updateComponent = async () => {
    await axios
      .get(`${serverConfig.url}events/${this.props.eventId}`)
      .then((res) => {
        if (this.props.user) {
          this.setState({
            userOwner: this.props.user.email === res.data.organiser.email,
          });
        }
        this.setState({
          isLoaded: true,
    ...
    axios
      .get(`${serverConfig.url}events/${this.props.eventId}/recommendations`)
      .then((res) => {
        this.setState({ recommendations: res.data });
      });

    axios
      .get(`${serverConfig.url}events/${this.props.eventId}/user-attending`)
      .then((result) => {
  ...
  onAttendChange = () => {
    axios
      .post(`${serverConfig.url}events/${this.props.eventId}/attend`, {
  ...
  remindMe = () => {
    axios
      .put(`${serverConfig.url}events/${this.props.eventId}/remind`, {
  ...
  updateAttendeesAmount = (amount) => {
    this.setState({
      attendeesAmount: this.state.attendeesAmount + amount,
    });
  };
          ...
          <Conditional if={this.state.error}>
            <div className="card-body p-5">
              <div className="d-flex align-items-center">
                <img
                  src="https://pluspng.com/img-png/kitten-png--243.png"
                  alt="Sad kitten"
                />
            ...
            <div className="card-body p-5">
              ...
              <div className="row">
                <div className="col-md-7">
                  <p>
                    {this.state.attendeesAmount} out of {this.state.capacity}{" "}
                    attending
                  </p>
                ...
                <div className="col-md-5">
                  <div className="card">
                    <div className="card-body">
                      ...
                      {this.state.organiser ? (
                        <p>
                          ...
                          <a href={`mailto:${this.state.organiser.email}`}>
                            {this.state.organiser.name ||
                              this.state.organiser.email}
                          </a>
                        </p>
                      ) : (
                        <p>No organiser</p>
                      ...
                      {this.state.zoomUrl ? (
                        <span>
                          ...
                          <p className="font-italic small">
                            Click link above to join meeting
                          </p>
                        </span>
                      ) : null}
                      {this.state.series ? (
                        <p>
                          Series:{" "}
                          <Link to={`/series/${this.state.series._id}`}>
                      ...
                      this.state.specificLocation.location &&
                      this.state.specificLocation.location.coordinates &&
                      this.state.specificLocation.location.coordinates
                        .length ? (
                        <div className="location-container">
                          <p className="location-label">Location</p>
                          <span className="specific-location-container">
                            <i className="location-marker fas fa-map-marker-alt" />
                            <span>
                              <span className="specific-location">
                                {this.state.specificLocation.name}
                              </span>
                              {this.state.specificLocation.address && (
                                <span className="specific-location-address">
                                  ( {this.state.specificLocation.address} )
                                </span>
                              )}
                            </span>
                          ...
                          <MapWithAMarker
                            coordinates={{
                              lat: this.state.specificLocation.location
                                .coordinates[0],
                              lng: this.state.specificLocation.location
                                .coordinates[1],
                            }}
                            containerElement={
                              <div
                    ...
                    <div className="card-footer text-center">
                      <AttendButton
                        full={
                          this.state.capacity === this.state.attendeesAmount
                        }
                      ...
                      <div className="mt-2">
                        <Conditional
                          if={
                            this.state.attending && this.props.user.phoneNumber
                          }
                        >
                          <RemindMeSwitch
                            reminding={this.state.reminding}
                  ...
                  <Conditional if={this.state.userOwner}>
                    <Link
                      to={`/events/admin/${this.props.eventId}`}
              ...
              <Conditional if={this.state.recommendations.length !== 0}>
                ...
                <SearchResults
                  results={this.firstThreeRecommendations()}
                  isLoading={false}
                />
              </Conditional>
